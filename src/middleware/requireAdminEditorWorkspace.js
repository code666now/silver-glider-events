...
async function requireAdminEditorWorkspace(req, res, next) {
  ...
  if (!dedicated) {
    ...
    return reject(res, 403, 'dedicated_admin_required',
      'Sign in through the dedicated admin login.');
  }

  const token = readAdminEditorCookie(req);
  if (!token) {
    return reject(res, 401, 'admin_editor_workspace_required',
  ...
  try {
    ...
    if (Number(row.actor_admin_operator_id) !== operatorId) {
      ...
      clearAdminEditorCookie(res);
      return reject(res, 403, 'admin_editor_operator_mismatch',
        'This editor workspace belongs to a different administrator.');
    }
    if (row.status !== 'active') {
      clearAdminEditorCookie(res);
      return reject(res, 401, 'admin_editor_workspace_inactive',
    ...
    if (new Date(row.expires_at).getTime() <= Date.now()) {
      ...
      clearAdminEditorCookie(res);
      return reject(res, 401, 'admin_editor_workspace_expired',
    ...
    if (row.operator_status !== 'active' || row.target_account_status !== 'active' ||
      ...
      clearAdminEditorCookie(res);
      return reject(res, 403, 'admin_editor_scope_unavailable',
