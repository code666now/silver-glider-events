...
async function createGuestSession(db, {
  identityId,
  userId,
  displayFirstName,
  displayName,
  ...
  const { rows } = await db.query(
    `INSERT INTO guest_sessions
       (identity_id,user_id,token_hash,display_first_name,display_name,verified_at,verified_event_id,expires_at)
     VALUES ($1,$2,$3,$4,$5,CASE WHEN $6 THEN NOW() ELSE NULL END,$7,NOW() + INTERVAL '180 days')
     RETURNING id,identity_id,user_id,display_first_name,display_name,verified_at,verified_event_id,expires_at`,
    [identityId, userId, tokenHash(token), firstName, fullName, verified, verified ? verifiedEventId : null]
  );
  return { ...rows[0], token };
}
...
async function readGuestSession(db, req, { touch = false } = {}) {
  ...
  const statement = touch
    ? `UPDATE guest_sessions gs SET last_seen_at=NOW()
        ...
        RETURNING gs.id,gs.identity_id,gs.user_id,gs.display_first_name,gs.display_name,
                  gs.verified_at,gs.verified_event_id,gs.expires_at,o.email`
    : `SELECT gs.id,gs.identity_id,gs.user_id,gs.display_first_name,gs.display_name,
              gs.verified_at,gs.verified_event_id,gs.expires_at,o.email
...
async function revokeIdentityGuestSessions(db, identityId, userId = null) {
  const { rowCount } = await db.query(
    `UPDATE guest_sessions SET revoked_at=NOW()
      WHERE (user_id=$2 OR (($2::int IS NULL OR user_id IS NULL) AND identity_id=$1))
