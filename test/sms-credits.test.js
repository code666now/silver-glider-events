...
test('Host Settings owns the feature while secrets remain server-only', () => {
  ...
  assert.match(settingsClient, /destination\.hostname !== 'checkout\.stripe\.com'/);
  assert.match(settingsClient, /Opening secure checkout/);
  assert.doesNotMatch(settingsClient, /PayPal|paypal|Venmo|venmo|payment-sheet|payment-progress/);
  assert.match(route, /requireOrganizer/);
  assert.match(route, /publicPacks\(\)/);
  assert.match(route, /packForKey\(req\.body\?\.packKey\)/);
  assert.match(route, /createCheckoutSession/);
  assert.match(route, /checkout\.stripe\.com/);
  assert.match(route, /createPendingPurchase\(pool, req\.organizer\.id, pack, 'stripe'\)/);
  assert.match(index, /express\.raw\(\{ type: 'application\/json'/);
  assert.match(index, /routes\/stripe-sms-webhook/);
  assert.match(webhook, /constructWebhookEvent/);
  assert.match(webhook, /checkout\.session\.completed/);
  assert.match(webhook, /session\.payment_status !== 'paid'/);
  assert.match(webhook, /packForPrice\(priceId\)/);
  assert.doesNotMatch(publicFiles, /STRIPE_SMS_SECRET_KEY|STRIPE_SMS_WEBHOOK_SECRET|STRIPE_SMS_\d+_PRICE_ID/);
});
