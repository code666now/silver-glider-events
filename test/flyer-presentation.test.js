...
test('flyer RSVP confirmation is selected without changing Standard confirmation output', () => {
  ...
  assert.doesNotMatch(standardHtml, /You(?:'|’)re on the list/);
  assert.match(standardHtml, /RSVP Confirmed/);
  ...
  assert.doesNotMatch(flyerHtml, /You(?:'|’)re on the list/);
  assert.match(flyerHtml, /RSVP Confirmed/);
