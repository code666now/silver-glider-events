...
- **SMS Go 2/3:** `src/lib/paypal.js`, `src/lib/sms-credit-ledger.js`, and the protected SMS routes provide fixed server-priced PayPal/Venmo credit packs plus a paid **Text guests tomorrow** action. On the day before an eligible event, the host reviews exact server-generated copy, masked opted-in recipients, segment cost, and balance before confirmation. Credits are reserved atomically, delivery is queued and audited, duplicate sends are blocked, and there is no free allowance or free-form composer.
